...
@if($request->segment(1) == 'pos' && ($request->segment(2) == 'create' || $request->segment(3) == 'edit' ||  $request->segment(2) == 'create_pc'))
    @php
        $pos_layout = true;
    @endphp
@else
...
<html lang="{{ app()->getLocale() }}" dir="{{in_array(session()->get('user.language', config('app.locale')), config('constants.langs_rtl')) ? 'rtl' : 'ltr'}}">
    ...
    use App\Product;
    $model =new Product();
    $expired = $model->where('is_inactive',1)->count();
    echo '<div class="alert alert-danger" role="alert">
  you have expired products'.$expired.'</div>'; 
